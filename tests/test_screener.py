...
class TestPassesHardFilters:
    def _call(self, symbol="TSLA", avg_volume=500_000, price=200.0, atr=6.0):
    ...
    def test_fails_atr_pct_too_high(self):
        # atr=12.0, price=200 → atr_pct=6% > 5%
        ok, meta = self._call(atr=12.0, price=200.0)
        assert ok is False

    def test_passes_atr_pct_at_new_cap(self):
        # atr=10.0, price=200 → atr_pct=5% — exactly at new cap, should pass
        ok, meta = self._call(atr=10.0, price=200.0)
        assert ok is True

    def test_passes_tsla_class_atr(self):
        # TSLA-like: atr_pct=4.3% — was excluded before, now passes
        ok, meta = self._call(atr=8.6, price=200.0)
        assert ok is True

    def test_beta_no_longer_a_filter(self):
        # Beta was unreliable on IEX — filter removed, any stock passes on other criteria
        ok, meta = self._call()
        assert ok is True
